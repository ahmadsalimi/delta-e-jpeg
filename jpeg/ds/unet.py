...
class UNetNaiveDownsample(UNet):

    def __init__(self, n_channels: int = 64,
                 ch_mults: Sequence[int] = (1, 2),
                 n_blocks: int = 2,
                 n_groups: int = 32):
        super().__init__(n_channels=n_channels, ch_mults=ch_mults, n_blocks=n_blocks, n_groups=n_groups)
        self.naive_downsample = NaiveDownsample()

    def forward(self, x: torch.Tensor):
        x = super().forward(x)
        return self.naive_downsample(x)


class UNetNaiveUpsample(UNet):
    ...
    def __init__(self, n_channels: int = 64,
                 ...
                 n_groups: int = 32):
        super().__init__(n_channels=n_channels, ch_mults=ch_mults, n_blocks=n_blocks, n_groups=n_groups)
        self.naive_upsample = NaiveUpsample()

    def forward(self, y: torch.Tensor, cbcr: torch.Tensor):
        x = self.naive_upsample(y, cbcr)
